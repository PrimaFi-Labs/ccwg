...
//ccwg/ccwg-web/src/types/database.ts
...
export interface Player {
  ...
  total_xp: number;
  stark_points: number;
  strk_balance: string; // bigint as string
  created_at: string;
  updated_at: string;
}
...
export interface PlayerCard {
  ...
  merge_count: number;
  acquired_at: string;
  is_new?: boolean; // Set by purchase response when template was not owned pre-purchase
  template?: CardTemplate; // Joined data
}
...
// UPDATED Database types - Replace the MarketItem and add PurchaseHistory in your src/types/database.ts
...
export interface MarketItem {
  ...
  price_strk: string; // bigint as string (wei)
  cards_granted: number;
  possible_cards: number[] | null; // Pool of template_ids for random selection
  guaranteed_cards: number[] | null; // Template_ids always included (hybrid mode)
  card_weights: Record<string, number> | null; // Weighted distribution: { "template_id": weight }
  image_url: string | null;
...
export interface PurchaseHistory {
  ...
  item_id: number;
  tx_hash: string;
  amount_paid: string; // bigint as string
  cards_received: number[]; // Array of player_card IDs granted
  purchased_at: string;
}

// Add this interface for the card distribution logic
export interface CardDistributionConfig {
  guaranteed: number[]; // Template IDs that are always granted
  randomPool: number[]; // Template IDs to randomly select from
  randomCount: number; // How many random cards to select
  weights?: Record<string, number>; // Optional weighted distribution
}
